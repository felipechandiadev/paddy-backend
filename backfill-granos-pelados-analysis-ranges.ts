...
async function backfillGranosPeladosAnalysisRanges() {
  ...
  try {
    ...
    const savedRows = await repository.find({
      where: { discountCode: 3 },
      order: { rangeStart: 'ASC' },
    });

    console.log(
      `✅ GRANOS PELADOS actualizado (discountCode 3). Rangos guardados: ${savedRows.length}`,
    );
    console.table(
      savedRows.map((row) => ({
